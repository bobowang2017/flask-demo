...
class ApiError(Exception):
    def __init__(self, value="UnKnow Reason", op="", data=None):
        ...
        self.op = str(op)
        if op:
            self.value = "{cls}.{op}: {value}".format(
                cls=self.__class__.__name__, op=self.op, value=self.value)
        else:
...
class ConfigParserError(ApiError):
    def __init__(self, value="Config Parser Error", data=None, op=""):
        super().__init__(value=value, op=op, data=data)


class LdapError(ApiError):
    def __init__(self, value="LDAP Error", data=None, op=""):
        super().__init__(value=value, op=op, data=data)


class JenkinsError(ApiError):
    def __init__(self, value="Jenkins Error", data=None, op=""):
        super().__init__(value=value, op=op, data=data)
...
class HarborError(ApiError):
    def __init__(self, value="Harbor Error", data=None, op=""):
        super().__init__(value, op, data)
...
class GitError(ApiError):
    def __init__(self, value="UnKnow Reason", op="UnKnow Operator", data=None):
        if isinstance(value, dict):
            value_default = defaultdict(lambda: "UnKnow Reason", value)
            value = '. '.join(["%s error: %s" % (k, ','.join(v))
                               for k, v in value_default.items() if v])
...
class AlertError(ApiError):
    def __init__(self, value="Alert Error", data=None, op=""):
        super().__init__(value, op, data)


class OWBEError(ApiError):
    def __init__(self, value="OWBE api Request Error", data=None, op=""):
        super().__init__(value, op, data)


class VDCError(ApiError):
    def __init__(self, value="VDC Request Error", data=None, op=""):
        super().__init__(value, op, data)


class WebSSHError(ApiError):
    def __init__(self, value="WebSSH Request Error", data=None, op=""):
        super().__init__(value, op, data)


class ClusterError(ApiError):
    def __init__(self, value="Cluster Request Error", data=None, op=""):
        super().__init__(value, op, data)
